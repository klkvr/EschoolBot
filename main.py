...
def send_lessons(user_id, s, lessons):
    msg = ''
    for i in range(8):
        msg += f'<b>{i}</b>. <i>{lessons[i]}</i>\n'
    user = BotUser(user_id)
    bot.send_message(user.id, msg, parse_mode="HTML")
...
@bot.message_handler(commands=['get_lessons'])
def get_lessons(message):
    try:
        ...
        if not user.logged_in:
            ...
            kb = create_calendar()
            user.state = 'get_lessons_choose_day'
            bot.send_message(user.id, choose_day_to_get_lessons, reply_markup=kb)
    except:
        bot.send_message('@eschool239boterrors', traceback.format_exc())
    ...
    try:
        ...
        if user.state == 'enter_login':
            ...
            if log_in_attempt['logged_in']:
                ...
                user.logged_in = True
                user.get_diary_units(log_in_attempt['session'])
