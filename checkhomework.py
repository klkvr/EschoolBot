...
diary = requests.get('https://app.eschool.center/ec-server/student/diary?userId=' + str(users_data[user_id]['id']) + '&d1=' + str(timee * 1000), cookies=s.cookies).json()['lesson']
bindiary = {}
for i in diary:
    date = i['date'] // 1000
    unit = i['unit']['name']
    if date not in bindiary:
        bindiary[date] = []
    if unit not in bindiary[date]:
        bindiary[date][unit] = []
    print(len(i['part']))
    for j in i['part']:
        print(len(j['variant']), j['variant'])